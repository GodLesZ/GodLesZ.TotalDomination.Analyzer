    ...
    public class JsonParser {
        ...
        private static void DeserializeType(IEnumerable<PropertyInfo> map, IDictionary<string, object> bag, object instance) {
            foreach (var info in map) {
                ...
                if (info.PropertyType == typeof(DateTime)) {
                    ...
                    var seconds = Int32.Parse(value.ToString(), NumberStyles.Number, CultureInfo.InvariantCulture);
        ...
        internal static void SerializeItem(StringBuilder sb, object item) {
            ...
            if (item is JsonObject) {
                SerializeObject((item as JsonObject).Hashmap, sb);
                return;
            }
        ...
        internal static KeyValuePair<string, object>? ParsePair(IList<char> data, ref int index) {
            ...
            if (!valid) {
                throw new InvalidJsonException(string.Format("Invalid JSON found while parsing a value pair at index {0}.", index));
            }
        ...
        internal static object ParseValue(IList<char> data, ref int index) {
            ...
            switch (token) {
                ...
                case JsonToken.Comma:
                    throw new InvalidJsonException(string.Format("Invalid JSON found while parsing a value at index {0}.", index));
                // Value Tokens
                case JsonToken.LeftBrace:
                    return ParseObject(data, ref index);
                case JsonToken.LeftBracket:
        ...
        internal static IDictionary<string, object> ParseObject(IList<char> data, ref int index) {
            ...
            while (index < data.Count - 1) {
                ...
                switch (token) {
                    ...
                    case JsonToken.Number:
                        throw new InvalidJsonException(string.Format("Invalid JSON found while parsing an object at index {0}.", index));
                    ...
                    case JsonToken.String:
                        ...
                        if (pair.HasValue) {
                            result.Add(pair.Value.Key, pair.Value.Value);
                        }
                        break;
                    default:
        ...
        internal static IEnumerable<object> ParseArray(IList<char> data, ref int index) {
            ...
            while (index < data.Count - 1) {
                ...
                switch (token) {
                    // End Tokens
                    case JsonToken.Unknown:             // Bad Data
                        throw new InvalidJsonException(string.Format("Invalid JSON found while parsing an array at index {0}.", index));
        ...
        internal static object ParseNumber(IList<char> data, ref int index) {
            ...
            if (!double.TryParse(buffer, JsonNumbers, CultureInfo.InvariantCulture, out result)) {
                throw new InvalidJsonException(string.Format("Value '{0}' was not a valid JSON number", buffer));
            }
